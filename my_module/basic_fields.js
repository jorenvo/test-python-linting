 ...
 */
  ...
  const DebouncedField = AbstractField.extend({
    ...
    init: function() {
      ...
      if (this.mode === 'edit') {
        ...
        const self = this;
        const debouncedFunction = this._doDebouncedAction;
  ...
  const FieldBoolean = AbstractField.extend({
    ...
    activate: function(options) {
      const activated = this._super.apply(this, arguments);
    ...
    /**
     * @override
     * @return {jQuery} the focusable checkbox input
     */
    getFocusableElement: function() {
      return this.$input || $();
    },
 ...
 **/
  const FieldFloatToggle = AbstractField.extend({
     ...
     *
     * @private
     * @return {float} The current formatted value
     */
     ...
     *
     * @private
     * @return {number} The next formatted value in the range
     */
    _nextValue: function() {
      const range = this.nodeOptions.range;
      const val = utils.closestNumber(this._getDisplayedValue(), range);
      const index = _.indexOf(range, val);
      if (index !== -1) {
 ...
 */
  ...
  const AbstractFieldBinary = AbstractField.extend({
    ...
    set_filename: function(value) {
      const filename = this.attrs.filename;
 ...
 */
  const FieldDomain = AbstractField.extend({
    ...
    _reset: function() {
      this._super.apply(this, arguments);
      const oldDomainModel = this._domainModel;
      this._setState();
 ...
 */
  const AceEditor = DebouncedField.extend({
     ...
     */
    _render: function() {
      const newValue = this._formatValue(this.value);
